...
def create_hook_empties(context, tracks, hooks):
    ...
    for t in tracks:
        ...
        t.select = True
        h.select = True
        context.scene.objects.active = t
...
def hook_em_up(hooks, ob):
    # create a to be assigned hook modifier for each hook in the list
    modlist=[]
    for h in hooks:
...
def setup_2d_reconstruction_cam(context, cams):
...
class VIEW3D_OT_reconstruct_2d_plane(bpy.types.Operator):
    bl_idname = "clip.reconstruct_2d_plane"
    bl_label = "Reconstruct 2d Plane"
    ...
    def execute(self, context):
        ...
        # setup the camera
        setup_2d_reconstruction_cam(context, cams)
...
class CLIP_PT_scene_reconstruction(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.operator("clip.reconstruct_2d_plane")
...
def register():
    bpy.utils.register_class(VIEW3D_OT_reconstruct_2d_plane)
    bpy.utils.register_class(VIEW3D_OT_reconstruct_3d_plane)
    bpy.utils.register_class(CLIP_PT_cscene_reconstruction)
...
def unregister():
    ...
    bpy.utils.unregister_class(VIEW3D_OT_reconstruct_2d_plane)
